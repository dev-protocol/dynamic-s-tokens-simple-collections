...
import type { MembersCollections } from '../typechain-types'
...
describe('MembersCollections', () => {
	...
	describe('removeImages', () => {
		describe('success', () => {
			it('remove the images', async () => {
				...
				expect((await cont.propertyImages(property.address, x)).slots).to.equal(
					slots
				)
	...
	describe('onBeforeMint', () => {
		describe('success', () => {
			it('returns true if receives the defined bytes32 key and passes member slot validation', async () => {
				const cont = await deployWithProxy<MembersCollections>(
					'MembersCollections'
				)
			...
			it('mints the members stoken & reduces available slots', async () => {
				const cont = await deployWithProxy<MembersCollections>(
					'MembersCollections'
				)
				const swapAndStake = await (
		...
		describe('fail', () => {
			it('should fail when slots are filled', async () => {
				const cont = await deployWithProxy<MembersCollections>(
					'MembersCollections'
				)
